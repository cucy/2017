...
class Connection:
    all_host = list(Asset.objects.values_list('public_ip'))
    def __init__(self):
        self.active_host = self.get_host_active()


    def get_host_active(self):
        # 检测9001端口是否已经打开
        host_active = []
        for host in self.all_host:
            ...
            sk.settimeout(1)
            try:
                sk.connect((host, 9001))
                host_active.append(host)
            except Exception as e:
                logger.warning('ip:%s 9001端口没有打开 %s' % (host, e.args) )
            sk.close()
            return host_active

    def put_pro_to_redis(self):
        pass
