...
# __all__  在 __init__ 可以只导入 列表里定义类 from . import *
...
class UserCreateView(AdminUserRequiredMixin, SuccessMessageMixin, CreateView):
    # SuccessMessageMixin 创建成功 flash新消息提示
    model = User
    form_class = forms.UserCreateUpdateForm
    template_name = 'users/user_create.html'
    success_url = reverse_lazy('users:user-list')

    success_message = _('Create user <a href="{url}">{name}</a> successfully.') #* 调用属性实例信息
    ...
    def form_valid(self, form):
        ...
        # 谁创建
        user.save()
        user_add_success_next(user) # 下一步动作
        return super(UserCreateView, self).form_valid(form)

    def get_success_message(self, cleaned_data):
        # success_message  定义  #* 调用属性实例信息 实例化以后才能得到信息 ， 创建哪个用户 成功信息
...
class UserUpdateView(AdminUserRequiredMixin, UpdateView):
    ...
    success_url = reverse_lazy('users:user-list') # 重定向到哪个url
